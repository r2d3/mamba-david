...
#include <c10/hip/HIPException.h>  // For C10_CUDA_CHECK and C10_CUDA_KERNEL_LAUNCH_CHECK
#include <ATen/hip/Atomic.cuh>  // For atomicAdd on complex
...
void selective_scan_bwd_kernel(SSMParamsBwd params) {
    ...
    using BlockReverseScanT = typename Ktraits::BlockReverseScanT;
    using BlockExchangeT = typename Ktraits::BlockExchangeT;
    using BlockReduceT = typename Ktraits::BlockReduceT;
    using BlockReduceFloatT = typename Ktraits::BlockReduceFloatT;
    using BlockReduceComplexT = typename Ktraits::BlockReduceComplexT;
    ...
    auto& smem_exchange = *reinterpret_cast<typename BlockExchangeT::TempStorage*>(smem_ + Ktraits::kSmemIOSize);
    auto& smem_exchange1 = *reinterpret_cast<typename BlockExchangeT::TempStorage*>(smem_ + Ktraits::kSmemIOSize + sizeof(typename BlockExchangeT::TempStorage));
    auto& smem_reduce = *reinterpret_cast<typename BlockReduceT::TempStorage*>(reinterpret_cast<char *>(&smem_exchange) + Ktraits::kSmemExchangeSize);
    auto& smem_reduce_float = *reinterpret_cast<typename BlockReduceFloatT::TempStorage*>(&smem_reduce);
    auto& smem_reduce_complex = *reinterpret_cast<typename BlockReduceComplexT::TempStorage*>(&smem_reduce);
    auto& smem_scan = *reinterpret_cast<typename BlockScanT::TempStorage*>(reinterpret_cast<char *>(&smem_reduce) + Ktraits::kSmemReduceSize);
    auto& smem_reverse_scan = *reinterpret_cast<typename BlockReverseScanT::TempStorage*>(reinterpret_cast<char *>(&smem_scan) + sizeof(typename BlockScanT::TempStorage));
    ...
    for (int chunk = params.n_chunks - 1; chunk >= 0; --chunk) {
        ...
        for (int state_idx = 0; state_idx < params.dstate; ++state_idx) {
            ...
            if constexpr (!kIsComplex) {
                ...
                BlockReverseScanT(smem_reverse_scan).InclusiveReverseScan(
                ...
                if constexpr (kIsVariableB || kIsVariableC) {
                    if constexpr (kIsVariableB) {
                        BlockExchangeT(smem_exchange).BlockedToStriped(dB_vals, dB_vals);
                    }
                    if constexpr (kIsVariableC) {
                        auto &smem_exchange_C = !kIsVariableB ? smem_exchange : smem_exchange1;
                        BlockExchangeT(smem_exchange_C).BlockedToStriped(dC_vals, dC_vals);
                    }
                ...
                if constexpr (!kIsVariableB || !kIsVariableC) {
                    ...
                    dA_dBC_val = BlockReduceT(smem_reduce).Sum(dA_dBC_val);
                    dA_val = dA_dBC_val.x;
                    if (threadIdx.x == 0) {
                        smem_dbc[state_idx] = chunk == params.n_chunks - 1 ? dA_dBC_val.y : dA_dBC_val.y + smem_dbc[state_idx];
                    }
                } else {
                    dA_val = BlockReduceFloatT(smem_reduce_float).Sum(dA_val);
                }
                if (threadIdx.x == 0) {
                ...
                if constexpr (kIsVariableB || kIsVariableC) {
                    ...
                    if constexpr (kIsVariableB) {
                        ...
                        BlockExchangeT(smem_exchange).BlockedToStriped(dB_vals_f, dB_vals_f);
                    }
                    if constexpr (kIsVariableC) {
                        #pragma unroll
                        for (int i = 0; i < kNItems; ++i) {
                        ...
                        auto &smem_exchange_C = !kIsVariableB ? smem_exchange : smem_exchange1;
                        BlockExchangeT(smem_exchange_C).BlockedToStriped(dC_vals_f, dC_vals_f);
                    }
                ...
                if constexpr (!kIsVariableB || !kIsVariableC) {
                    ...
                    dA_val = BlockReduceComplexT(smem_reduce_complex).Sum(dA_val);
                }
                if (threadIdx.x == 0) {
                    smem_da[state_idx] = chunk == params.n_chunks - 1 ? dA_val : dA_val + smem_da[state_idx];
                }
    ...
    if (params.dD_ptr != nullptr) {
        dD_val = BlockReduceFloatT(smem_reduce_float).Sum(dD_val);
        if (threadIdx.x == 0) { gpuAtomicAdd(dD, dD_val); }
    }
    if (params.ddelta_bias_ptr != nullptr) {
        __syncthreads();
        ddelta_bias_val = BlockReduceFloatT(smem_reduce_float).Sum(ddelta_bias_val);
